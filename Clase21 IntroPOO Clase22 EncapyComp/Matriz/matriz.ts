...
class Nodo {
    ...
    constructor(fila: number, columna: number, valor: number) {
        this.fila = fila;
...
let getElem = (fila: number, columna: number, matriz: Array<Nodo>): number => { 
    let nodo: Nodo = matriz[matriz.length-1]; //Tomo el ultimo NODO para saber la cant de FILAS y COLUMNAS
...
let cargarMatriz = (arreglo: Array<Array<number>>): Array<Nodo>  => { 
    let matriz: Array<Nodo>  = [];
...
let matriz: Array<Nodo> = cargarMatriz(arregloPrueba);
...
let valor: number = getElem(aux1,aux2,matriz);
